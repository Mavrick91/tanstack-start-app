...
export const createCheckout = async (input) => {
  if (!items) return { success: false, error: 'Cart items required', status: 400 }
  return { success: true, checkout }
}

export const createCheckoutFn = createServerFn()
  .handler(async ({ data }) => {
...
export async function fetchProductRelations(productIds: string[]): Promise<ProductRelations> {
